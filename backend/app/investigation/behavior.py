...
def _event_ids_for_cmd(
    timeline: AttackTimeline, snapshots: list[dict[str, Any]], patterns: list[str]
) -> list[str]:
    ids = []
    for snap in snapshots:
        proc = snap.get("process") or {}
...
def _check_privilege_escalation(
    ...
    high_sev_procs = [
        e.event_id for e in timeline.entries if e.severity >= 7 and e.category == "process"
    ]
...
def _check_command_and_control(
    ...
    beaconing_proc = next((proc for proc, cnt in proc_counts.items() if cnt >= 3), None)
...
def _check_ransomware(
    ...
    all_hits = list(
        dict.fromkeys(
            proc_hits + cmd_hits + ([e.event_id for e in file_events[:10]] if file_vol_hit else [])
        )
    )
    if not all_hits:
        return None
    ...
    confidence = min(
        0.5 + (0.2 if proc_hits else 0) + (0.3 if cmd_hits else 0) + (0.15 if file_vol_hit else 0),
        0.95,
    )
    first, last = _min_max_ts(timeline, all_hits)
    return DetectedBehavior(
