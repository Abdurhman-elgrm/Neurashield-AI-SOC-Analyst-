"""
Demo mode service â€” creates the demo tenant + user if not present,
...
# â”€â”€â”€ Demo constants â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def demo_login(db: AsyncSession) -> TokenPair:
    ...
    # Issue tokens â€” skip all credential checks
...
# â”€â”€â”€ Tenant / user helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Wipe â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _wipe_tenant_data(db: AsyncSession, tenant_id: str) -> None:
    ...
    # playbook_steps has no tenant_id â€” cascade-deleted via playbooks FK
    for stmt in [
...
# â”€â”€â”€ Master seed â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
RULE_DEFS = [
    ...
    (
        "Brute Force â€” SSH Login Failures",
        "high",
        "threshold",
        ["credential-access"],
        ["T1110.001"],
    ...
    (
        "Failed Admin Login â€” Multiple Hosts",
        "high",
        "threshold",
        ["lateral-movement"],
        ["T1021"],
...
async def _seed_agents(db: AsyncSession, tid) -> list[Agent]:
    ...
    for hostname, os_type, status, version, ip, tags in AGENT_DEFS:
        a = Agent(
            ...
            name=hostname,
            hostname=hostname,
            os_type=AgentOsType(os_type),
            status=AgentStatus(status),
            agent_version=version,
            ip_address=ip,
            enrollment_token_hash=f"hmac_sha256:{secrets.token_hex(32)}",
            last_seen_at=(
                _ago(minutes=random.randint(1, 120))
                if status == "online"
                else _ago(hours=random.randint(3, 72))
            ),
            tags=tags,
            config={},
...
ALERT_TEMPLATES = [
    (
        "Mimikatz Credential Dumping on {host}",
        "critical",
        ["credential-access"],
        ["T1003.001"],
        "open",
    ),
    ("Ransomware Encryption Activity â€” {host}", "critical", ["impact"], ["T1486"], "open"),
    (
    ...
    (
        "Kerberoasting Attack â€” Service Ticket Requested",
        "critical",
        ["credential-access"],
        ["T1558.003"],
        "open",
    ...
    (
        "Data Exfiltration via HTTP POST from {host}",
        "critical",
        ["exfiltration"],
        ["T1041"],
        "open",
    ...
    (
        "WMI Execution â€” Suspicious Child Process on {host}",
        "high",
        ["execution"],
        ["T1047"],
        "open",
    ),
    ("Port Scan from {host} â€” 1,240 ports", "high", ["discovery"], ["T1046"], "acknowledged"),
    (
        "SSH Brute Force on {host} â€” 847 attempts",
        "high",
        ["credential-access"],
        ["T1110.001"],
        "open",
    ...
    (
        "Process Injection into svchost.exe on {host}",
        "high",
        ["defense-evasion"],
        ["T1055"],
        "open",
    ...
    (
        "Suspicious DLL Load by chrome.exe on {host}",
        "high",
        ["defense-evasion"],
        ["T1574"],
        "closed",
    ...
    (
        "Admin Login Failure â€” 12 hosts in 5 minutes",
        "high",
        ["lateral-movement"],
        ["T1021"],
        "open",
    ...
    (
        "DNS Tunneling â€” Encoded queries on {host}",
        "medium",
        ["command-and-control"],
        ["T1071.004"],
        "open",
    ...
    (
        "Outbound C2 Connection on Port 4444 from {host}",
        "medium",
        ["command-and-control"],
        ["T1571"],
        "open",
    ...
    (
        "Scheduled Task Created: WindowsUpdate.exe on {host}",
        "medium",
        ["persistence"],
        ["T1053.005"],
        "open",
    ...
    (
        "Registry Run Key Added â€” Unknown Process on {host}",
        "medium",
        ["persistence"],
        ["T1547.001"],
        "acknowledged",
    ...
    (
        "Suspicious Parent-Child: explorer.exe â†’ cmd.exe",
        "medium",
        ["execution"],
        ["T1204"],
        "open",
    ...
    (
        "Failed Authentication â€” 23 attempts on {host}",
        "medium",
        ["credential-access"],
        ["T1110"],
        "open",
    ...
    (
        "Outbound Traffic to IOC: 185.220.101.x",
        "medium",
        ["command-and-control"],
        ["T1041"],
        "open",
    ...
    (
        "Cleartext Credentials in Process Args on {host}",
        "low",
        ["credential-access"],
        ["T1552.004"],
        "false_positive",
...
async def _seed_alerts(
    db: AsyncSession, tid, rules: list[DetectionRule], agents: list[Agent]
) -> list[Alert]:
    ...
    for title_tpl, sev, tactics, techniques, status_val in ALERT_TEMPLATES:
        for _ in range(random.randint(3, 6)):
            ...
            a = Alert(
                ...
                ai_metadata={
                    "ai_analysis": {
                        "summary": f"Alert on {agent.hostname} â€” {tactics[0]} via {techniques[0]}.",
...
# â”€â”€â”€ Investigations â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
INV_TEMPLATES = [
    (
        "APT29 Lateral Movement Campaign â€” Finance Dept",
        95,
        "high",
        "investigating",
        "true_positive",
    ...
    ("Credential Harvesting via Mimikatz â€” 3 Hosts", 88, "high", "triaged", None),
    ("C2 Beacon to 185.220.101.47 â€” Cobalt Strike", 85, "high", "active", "true_positive"),
    ...
    (
        "Kerberoasting Attack â€” Multiple Service Accounts",
        90,
        "high",
        "contained",
        "true_positive",
    ),
    ("Suspicious WMI Execution Chain â€” WEB-SRV-01", 71, "medium", "active", None),
    ("Data Exfiltration Attempt â€” FINANCE-WS-01", 87, "high", "resolved", "true_positive"),
    ("USB-Based Malware â€” HR-LAPTOP-01", 55, "low", "false_positive", "false_positive"),
    ("DNS Tunneling â€” Periodic Beacons Detected", 68, "medium", "triaged", None),
    ("Registry Persistence â€” SALES-WS-03", 49, "low", "new", None),
]


async def _seed_investigations(
    db: AsyncSession, tid, uid, alerts: list[Alert]
) -> list[Investigation]:
    invs = []
        ...
        inv = Investigation(
            ...
            graph_json={
                ...
                "edges": [
                    {
                        "source": f"host-{i % 7}",
                        "target": f"user-{random.randint(0, 2)}",
            ...
            ai_analysis_json={
                ...
                "mitre_coverage": [
                    {"tactic": b, "techniques": [f"T{random.randint(1001, 1600)}"]}
                    for b in behaviors
                ],
...
async def _seed_playbooks(
    db: AsyncSession, tid, uid, invs: list[Investigation], alerts: list[Alert]
) -> None:
        ...
        for i, (step_title, cat, step_status) in enumerate(_PLAYBOOK_STEPS):
            db.add(
                PlaybookStep(
                    ...
                    category=cat,
                    title=step_title,
                    description=f"Step {i + 1}: {step_title} â€” follow standard runbook procedure.",
                    status=step_status if is_active else "pending",
                    requires_human_approval=True,
                    completed_at=_ago(hours=random.randint(1, 12))
                    if step_status == "completed" and is_active
                    else None,
                )
            )
...
# â”€â”€â”€ Suppression rules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


async def _seed_suppression_rules(db: AsyncSession, tid, uid, rules: list[DetectionRule]) -> None:
    defs = [
        (
            "Suppress USB alerts on exec laptops",
            "USB known-good on C-suite devices",
            rules[18].id,
            "EXEC-LAPTOP*",
            "low",
            True,
        ...
        (
            "Mute legacy server scan alerts",
            "Legacy-01 generates false positives",
            rules[5].id,
            "LEGACY*",
            "medium",
            True,
        ...
        (
            "Dev environment false positives",
            "CI/CD builds trigger port scan rules",
            rules[5].id,
            "BUILD-AGENT*",
            "medium",
            True,
        ...
        (
            "HR endpoint â€” maintenance window",
        ...
        (
            "Sales team â€” VPN reconnect noise",
            "VPN client reconnects generate alerts",
            rules[6].id,
            "SALES-WS*",
            "low",
            True,
...
_AUDIT = [
    ...
    (
        "alert.closed",
        "alert",
        {"before": {"status": "acknowledged"}, "after": {"status": "closed"}},
    ),
    (
        "investigation.status_change",
        "investigation",
        {"before": {"status": "new"}, "after": {"status": "investigating"}},
    ),
    ...
    (
        ...
        "alert",
        {"before": {"status": "open"}, "after": {"status": "false_positive"}},
    ),
