...
def _fleet_status(agent: Agent) -> str:
    ...
    if agent.last_seen_at is not None:
        age = datetime.now(tz=timezone.utc) - agent.last_seen_at
...
async def list_fleet_agents(
    ...
    if hostnames:
        alert_q = (
            select(
                Alert.source_host,
                func.sum(case((Alert.severity == AlertSeverity.CRITICAL.value, 1), else_=0)).label("crit"),
                func.sum(case((Alert.severity == AlertSeverity.HIGH.value,     1), else_=0)).label("high"),
                func.sum(case((Alert.severity == AlertSeverity.MEDIUM.value,   1), else_=0)).label("med"),
                func.sum(case((Alert.severity == AlertSeverity.LOW.value,      1), else_=0)).label("low"),
    ...
    stale_count_q = await db.execute(
        select(func.count()).where(
            ...
            Agent.last_seen_at >= datetime.now(tz=timezone.utc) - timedelta(hours=_STALE_THRESHOLD_HOURS),
        )
    )
    ...
    needs_update = sum(
        1 for a in agents
        if (a.agent_version or "") != _CURRENT_AGENT_VERSION
    )
    ...
    return APIResponse.ok(FleetListResponse(
        agents=fleet_agents,
        stats=FleetStats(
            total=total_fleet,
            online=online_count,
...
async def get_fleet_stats(
    ...
    critical_active: int = (await db.execute(
        select(func.count()).select_from(Alert).where(
            Alert.tenant_id == tenant_id,
            Alert.severity == AlertSeverity.CRITICAL.value,
            Alert.status == AlertStatus.OPEN.value,
    ...
    needs_update: int = (await db.execute(
        select(func.count()).where(
            Agent.tenant_id == tenant_id,
            Agent.deleted_at.is_(None),
    ...
    return APIResponse.ok(FleetStats(
        total=total,
        online=online,
...
async def get_version_distribution(
    ...
    return APIResponse.ok([
        VersionDistribution(version=row.agent_version, count=row.count)
        for row in rows
    ])
