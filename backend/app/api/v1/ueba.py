...
async def get_top_risky_users(
    ...
    alert_map: dict[str, int] = {u: 0 for u in usernames}
...
async def get_user_timeline(
    ...
    return APIResponse.ok([
        UEBARiskPoint(date=str(r.day), score=round(float(r.avg_score), 3))
        for r in rows
    ])
...
async def get_flag_distribution(
    ...
    return APIResponse.ok([
        UEBAFlagCount(flag=r.flag, count=int(r.cnt))
        for r in rows
    ])
...
async def get_impossible_travel(
    ...
    for username, events in by_user.items():
        ...
        for i in range(len(events) - 1):
            ...
            entries.append(ImpossibleTravelEntry(
                username=username,
