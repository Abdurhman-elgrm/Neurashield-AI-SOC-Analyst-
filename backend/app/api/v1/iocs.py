...
async def _enrich_ip(indicator: str) -> IOCEnrichment:
    ...
    try:
        ...
        return IOCEnrichment(indicator=indicator, type="ip", vt_malicious=None,
                             vt_suspicious=None, vt_total=None, vt_verdict=None,
                             abuseipdb_confidence=None, abuseipdb_country=None,
                             first_seen=None, last_seen=None, tags=[])
...
async def _enrich_hash(indicator: str) -> IOCEnrichment:
    ...
    try:
        ...
        return IOCEnrichment(indicator=indicator, type="hash", vt_malicious=None,
                             vt_suspicious=None, vt_total=None, vt_verdict=None,
                             abuseipdb_confidence=None, abuseipdb_country=None,
                             first_seen=None, last_seen=None, tags=[])
...
async def enrich_ioc(
    ...
    return APIResponse.ok(IOCEnrichment(
        indicator=indicator,
        type=ioc_type,
        vt_malicious=None,
        vt_suspicious=None,
...
@router.get("/investigations/{investigation_id}/iocs",
            response_model=APIResponse[InvestigationIOCsResponse])
async def get_investigation_iocs(
    ...
    for r in rows:
        if r.source_ip: seen_ips.add(r.source_ip)
        if r.dest_ip:   seen_ips.add(r.dest_ip)
        if r.domain:    seen_domains.add(r.domain)
        if r.file_hash: seen_hashes.add(r.file_hash)
        if r.proc_name: seen_processes.add(r.proc_name)
    ...
    domain_list = [
        IOCEnrichment(indicator=d, type="domain", vt_malicious=None, vt_suspicious=None,
                      vt_total=None, vt_verdict="unknown", abuseipdb_confidence=None,
                      abuseipdb_country=None, first_seen=None, last_seen=None, tags=[])
        for d in list(seen_domains)[:20]
    ]
    process_list = [
        IOCEnrichment(indicator=p, type="process", vt_malicious=None, vt_suspicious=None,
                      vt_total=None, vt_verdict="unknown", abuseipdb_confidence=None,
                      abuseipdb_country=None, first_seen=None, last_seen=None, tags=[])
        for p in list(seen_processes)[:20]
    ]

    return APIResponse.ok(InvestigationIOCsResponse(
