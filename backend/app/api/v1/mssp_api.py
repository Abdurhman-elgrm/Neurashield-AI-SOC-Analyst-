...
async def get_mssp_overview(
    ...
    for tid in tenant_ids:
        ...
        alert_row = (await db.execute(
            select(
                ...
                func.min(Alert.created_at).filter(
                    Alert.severity == AlertSeverity.CRITICAL.value,
                    Alert.status   == AlertStatus.OPEN.value,
        ...
        unresolved: int = (await db.execute(
        ...
        # Online agents
        agents_online: int = (await db.execute(
            select(func.count()).where(
                Agent.tenant_id == tid,
        ...
        last_event_row = (await db.execute(
            text("SELECT MAX(event_timestamp) FROM events WHERE tenant_id = CAST(:tid AS uuid)"),
            {"tid": tid_str},
        )).scalar_one()
        last_event_at = last_event_row.isoformat() if last_event_row else None

        health_cards.append(TenantHealthCard(
            tenant_id=tid_str,
    ...
    return APIResponse.ok(MSSPOverviewResponse(
...
async def create_mssp_tenant(
    ...
    return APIResponse.ok(CreateTenantResponse(
