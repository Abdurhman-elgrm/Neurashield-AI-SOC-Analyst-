...
from app.detection.sigma import parse_sigma_yaml, save_sigma_rule, bulk_import_defaults, generate_sigma_rule
...
class GenerateRequest(BaseModel):
    ...
    category_hint: str | None = Field(default=None,
                                      description="Optional logsource category hint")
    severity_hint: str | None = Field(default=None,
                                      description="Optional severity level hint")
...
async def generate_rule(
    ...
    return APIResponse.ok(GenerateResult(
...
async def generate_and_import_rule(
    ...
    if not result.success:
        return APIResponse.ok(GenerateAndImportResult(
            rule_id="", title=result.parsed.title, severity=result.parsed.severity,
            category=result.parsed.category, mitre_techniques=[], mitre_tactics=[],
            conditions_count=0, yaml_text=result.yaml_text, created=False,
