...
class Investigation(Base, TimestampMixin):
    ...
    investigation_group_id: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    ...
    tp_probability: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    fp_probability: Mapped[float] = mapped_column(Float, nullable=False, default=1.0)
    ...
    attack_progression: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    ...
    assigned_to: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True, index=True)
    verdict: Mapped[str | None] = mapped_column(String(32), nullable=True)
    verdict_set_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    verdict_set_by: Mapped[UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
    ...
    triggering_alert_ids: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    ...
    def __repr__(self) -> str:
        return (
            f"<Investigation id={self.id} score={self.threat_score} confidence={self.confidence}>"
        )
