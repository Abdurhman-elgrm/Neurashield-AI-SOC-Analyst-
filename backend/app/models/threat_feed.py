...
class ThreatFeed(Base, TimestampMixin, SoftDeleteMixin):
    ...
    type: Mapped[FeedType] = mapped_column(
        Enum(FeedType, name="threat_feed_type_enum",
    ...
    status: Mapped[FeedStatus] = mapped_column(
        Enum(FeedStatus, name="threat_feed_status_enum",
             values_callable=lambda x: [e.value for e in x]),
        nullable=False,
    ...
    iocs: Mapped[list["ThreatIOC"]] = relationship(
        "ThreatIOC", back_populates="feed", lazy="noload"
    )

    __table_args__ = (
        Index("idx_threat_feed_tenant", "tenant_id"),
    )
...
class ThreatIOC(Base, TimestampMixin):
    ...
    type: Mapped[IOCType] = mapped_column(
        Enum(IOCType, name="threat_ioc_type_enum",
             values_callable=lambda x: [e.value for e in x]),
        nullable=False,
        index=True,
    )
