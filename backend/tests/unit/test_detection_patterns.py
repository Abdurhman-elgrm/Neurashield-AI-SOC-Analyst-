...
from app.normalization.models import NormalizedEvent, NormalizedProcess, NormalizedNetwork
...
class TestOperators:
    ...
    def test_contains(self):
        ...
        event = _make_event(process=proc)
        assert evaluate_condition({"field": "process.command_line", "op": "contains", "value": "EncodedCommand"}, event)

    def test_regex_match(self):
        proc = NormalizedProcess(name="svchost.exe")
        event = _make_event(process=proc)
        assert evaluate_condition({"field": "process.name", "op": "regex", "value": r"svc.*\.exe"}, event)

    def test_in_operator(self):
        proc = NormalizedProcess(name="cmd.exe")
        event = _make_event(process=proc)
        assert evaluate_condition({"field": "process.name", "op": "in", "value": ["cmd.exe", "powershell.exe"]}, event)

    def test_not_in_operator(self):
        proc = NormalizedProcess(name="notepad.exe")
        event = _make_event(process=proc)
        assert evaluate_condition({"field": "process.name", "op": "not_in", "value": ["cmd.exe", "powershell.exe"]}, event)
    ...
    def test_none_field_returns_false(self):
        event = _make_event(process=None)
        assert not evaluate_condition({"field": "process.name", "op": "eq", "value": "cmd.exe"}, event)
